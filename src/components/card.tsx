...
export function Card({ imageUrl, title, author }: CardProps) {
  return (
    <div className="relative w-96 h-96 flex items-end pb-8 gap-4 rounded-lg p-4 group overflow-hidden">
      <div className="flex flex-col z-20 text-white">
        <h2 className="w-80 font-bold text-lg">{title}</h2>
      ...
      <Image
        ...
        width={384}
        height={384}
        className="rounded-md absolute top-0 left-0 w-full h-full group-hover:scale-105 transition-all duration-300"
      />
    </div>
  );
}
